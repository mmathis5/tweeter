...
const handlerImpl = async (request, daoFactory) => {
    ...
    const [followerCount, followeeCount] = await followService.unfollow(request.token, tweeter_shared_1.User.fromDto(request.user), tweeter_shared_1.User.fromDto(request.userToFollowOrUnfollow));
    return {
        success: true,
        message: null,
